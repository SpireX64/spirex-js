...
export type EventListenerFunction<
    ...
    EventKey extends AnyEvent | keyof EventsMap,
> = (
    eventArgs: EventKey extends keyof EventsMap
        ? EventsMap[EventKey]
        : EventsMap[keyof EventsMap],
    event: EventKey,
) => Promise<void> | void;

export class EventBus<T extends object = Record<string, unknown>> {
    private readonly _listenersMap = new Map<
        keyof T | AnyEvent,
        Set<EventListenerFunction<T, any>>
    >();
    ...
    public emit<Key extends keyof T>(
        event: Key,
        data: T[Key],
        ...
        if (event !== ANY_EVENT_KEY)
            this._listenersMap
                .get(ANY_EVENT_KEY)
                ?.forEach((it) => it(data as any, event));
        if (preserve) {
    ...
    private add<TKey extends keyof T | AnyEvent>(
        ...
        if (this._listenersMap.has(event))
            ...
            this._listenersMap.set(
                event,
                new Set<EventListenerFunction<T, any>>().add(listener),
            );
